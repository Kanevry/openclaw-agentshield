...
- **366 tests passing** — scanner corpus validation, audit log tests, hook integration tests, typoglycemia detection, hex encoding, HTML exfiltration, rate anomaly detection, ROT13, markdown exfiltration, SSRF, path traversal, API key detection, dashboard HTML + routes. TypeScript strict mode, zero `any` types.
...
- **142+ detection patterns** across 16 categories, with base64, hex, and ROT13 decoding as pre-processing layers. The scanner catches attacks that bypass simple string matching.
...
- **70 attack corpus test cases** covering injection, exec abuse, write abuse, indirect injection, stealth attacks, and benign content (to verify we don't over-block).
