...
Under the hood, AgentShield runs 130+ detection patterns across 15 categories: instruction overrides, identity manipulation, credential extraction, markup injection, jailbreaks, data exfiltration, SSRF, path traversal, markdown exfiltration, HTML exfiltration, ROT13 obfuscation, system prompt extraction, and 22 API key/secret formats. It decodes base64, hex, and ROT13-encoded payloads, normalizes unicode, strips zero-width characters, and detects typoglycemia (scrambled-letter) evasion attacks — aligned with the OWASP LLM Prompt Injection Prevention Cheat Sheet. Every scan result streams in real-time to a live HTML dashboard via Server-Sent Events. The agent itself can call `shield_scan` and `shield_audit` as registered tools to proactively assess threats and query the audit log.
...
- **340 tests passing** — scanner corpus validation, audit log tests, hook integration tests, typoglycemia detection, hex encoding, HTML exfiltration, rate anomaly detection, ROT13, markdown exfiltration, SSRF, path traversal, API key detection, dashboard HTML + routes. TypeScript strict mode, zero `any` types.
...
- **130+ detection patterns** (142 total primitives) across 15 categories, with base64, hex, and ROT13 decoding as pre-processing layers. The scanner catches attacks that bypass simple string matching.
...
- **60 attack corpus test cases** covering injection, exec abuse, write abuse, indirect injection, stealth attacks, and benign content (to verify we don't over-block).
